...
@RestController // 데이터 response 하는 컨트롤러 -> 첨부파일하기 위한 controller
...
public class AttachmentController {
  ...
  public ResponseEntity<?> download(Long id){ // ResponseEntity<?> : 와일드 카드로 아무거나 다 받을 수 있음
    if (id == null) return new ResponseEntity<>(null, HttpStatus.BAD_REQUEST);  // 400 에러 발생

    Attachment file = attachmentService.findById(id);
    if (file == null) return new ResponseEntity<>(null, HttpStatus.NOT_FOUND);  // 404 에러  발생
    ...
    try {
      ...
      // 파일 유형이 지정되지 않은 경우
      if (mimeType == null){
        mimeType = "application/octet-stream";  // 일련의 byte 스트림 타임. 유형이 알려지지 않은 경우 지정하기
      }
      ...
      return new ResponseEntity<>(resource, headers, HttpStatus.OK);
        ...
        return new ResponseEntity<>(null, null, HttpStatus.CONFLICT); // 409 에러 발생
    }
